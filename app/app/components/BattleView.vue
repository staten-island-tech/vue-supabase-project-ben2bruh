  ...
  <div class="battle-modal">
    ...
    <div class="battle-field">
      ...
      <section v-if="activeEnemy" class="pokemon-card enemy-card">
        ...
        <img class="pokemon-sprite enemy-sprite" :src="activeEnemy.sprite" :alt="activeEnemy.name" />
      </section>

      <div v-if="activePlayer" class="battle-platform player-platform"></div>

      <section v-if="activePlayer" class="pokemon-card player-card">
        <img class="pokemon-sprite player-sprite" :src="activePlayer.sprite" :alt="activePlayer.name" />
        <div class="pokemon-meta">
    ...
    <div class="bottom-action-bar">
      ...
      <div class="action-panel">
          ...
          <div class="pokemon-switch-grid">
            ...
            >
              ...
              <div v-if="member" class="slot-hp-bar">
                <div class="slot-hp-fill" :style="{ width: `${getTeamHpPercent(member)}%` }"></div>
              </div>
            </button>
          </div>

          <button v-if="!awaitingSwitch" class="move-btn close-btn" type="button" @click="activeCommand = 'moves'">
            Back
          </button>
        </template>
        ...
        <template v-else-if="false">
          <button
            v-for="member in switchCandidates"
            :key="member.id ?? member.team_slot ?? member.name"
            class="move-btn switch-btn"
            type="button"
            @click="switchToPokemon(member)"
          >
            <span>{{ member.name }}</span>
            <small>Lv {{ member.level }} / {{ member.current_hp }} HP</small>
          </button>

          <div v-if="switchCandidates.length === 0" class="panel-note">
            No healthy Pokémon left.
          </div>
        </template>
        ...
        <button class="move-btn close-btn" type="button" :disabled="!canClose" @click="$emit('cancel')">
          Close
        </button>
      </div>
    </div>
...
const switchCandidates = computed(() => {
  ...
  return (props.team ?? [])
    .filter((member) => {
      ...
      return member.status_condition !== 'fainted' && Number(member.current_hp ?? 0) > 0
    })
})
...
const canClose = computed(() => isBattleOver.value && !awaitingSwitch.value && !choosingReward.value)
...
const displayFloor = computed(() => currentFloor.value || props.floorNumber || 1)
const roguelikeLoopEnabled = computed(() => Boolean(props.runId))

onMounted(async () => {
  if (!props.player) return

  const firstEnemy = props.runId
    ? await startRun(props.runId)
    : props.enemy
...
async function switchToPokemon(member: Record<string, any>) {
  if (!member || !awaitingSwitch.value || member.status_condition === 'fainted' || Number(member.current_hp ?? 0) <= 0) return
...
async function handleEnemyFainted() {
  if (!roguelikeLoopEnabled.value || !activePlayer.value || !activeEnemy.value) return
...
async function chooseReward(choice: Omit<ItemReward, 'run_id'>) {
  if (!choosingReward.value || !activePlayer.value || !activeEnemy.value) return
...
.battle-field {
  ...
  min-height: 0;
  background:
    linear-gradient(180deg, #7ec8e3 0 45%, #3a9e4f 45% 100%);
}
