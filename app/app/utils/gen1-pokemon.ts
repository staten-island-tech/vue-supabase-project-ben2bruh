/**
 * Gen 1 Pokédex with base stats, types, and evolution info
 * Includes the three starters (Bulbasaur, Charmander, Squirtle) and their evolution lines
 */
...
export interface Gen1Species {
  ...
  evolvesInto?: number // species_id of evolved form
  spriteUrl: string
}

// Core starters and select Gen 1 Pokémon for encounters (expanded set for variety)
export const gen1Pokemon: Record<number, Gen1Species> = {
  ...
  26: {
    ...
    name: 'Raichu',
    types: ['electric'],
    baseStats: { hp: 60, attack: 90, defense: 55, spAtk: 90, spDef: 80, speed: 100 },
...
// Eevee - special case with manual evolution choice
export const eeveeSpecies: Gen1Species = {
  id: 133,
  name: 'Eevee',
  types: ['normal'],
...
export function getWildPokemonPool(floor: number): Gen1Species[] {
  ...
  if (floor <= 5) {
    return allPokemon.filter(p => 
      (p.baseStats.hp + p.baseStats.attack + p.baseStats.defense) <= 150
    )
  } else if (floor <= 10) {
    return allPokemon.filter(p =>
      (p.baseStats.hp + p.baseStats.attack + p.baseStats.defense) <= 250
    )
  } else {
    return allPokemon
  }
...
export function calculateStat(baseStat: number, level: number, isHp: boolean = false): number {
  if (isHp) {
    return Math.floor(((2 * baseStat + 0) * level) / 100 + level + 5)
  }
  return Math.floor(((2 * baseStat + 0) * level) / 100 + 5)
}
